...
class TeamLogic extends LogicBase
{
    ...
    {
        self::$integral_switch = ConfigServer::get('marketing', 'integral_deduction_status', 0);
    ...
    public static function calculateInfo($post, $user_id)
    {
        try{
            ...
            if (isset($post['coupon_id'])){
                //订单优惠金额
                $discount_amount_data = OrderLogic::calculateDiscountAmount($goods_lists, $post['coupon_id']);
            ...
            if ($use_integral == 1 && $order_amount > 0){
                $integral_data = OrderLogic::integralDeductionMoney($goods_lists, $user_id, self::$integral_switch, self::$integral_config);
