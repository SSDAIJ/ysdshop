...
class OrderLogic extends LogicBase
{
    ...
    //订单类型
    private static $order_type  = \app\common\model\Order::NORMAL_ORDER;
        ...
        try{
            ...
            foreach ($goods as $good) {
                ...
                if (isset($seckill_goods[$good['item_id']])){
                    ...
                    self::$order_type = \app\common\model\Order::SECKILL_ORDER;//秒杀订单
                }
    ...
    //积分抵扣
    public static function integralDeductionMoney($goods_lists, $user_id, $switch, $integral_config)
    {
        $data = [
            'goods' => $goods_lists,
            'integral_money' => 0,
        ...
        $user_integral = round($user['user_integral'] * $integral_config, 2);//用户可抵扣积分金额
        ...
        if ($user_integral >= $total_price){
            $user_use_integral = $total_price / $integral_config;
    ...
    public static function addOrderGoods($order_id, $goods_lists)
    {
        foreach ($goods_lists as $k1 => $good) {
            ...
            $goods_data[] = [
                ...
                'total_pay_price' => ($good['goods_price'] * $good['goods_num']) - ($good['discount_price'] ?? 0),//实际支付商品金额(扣除优惠金额)
                'spec_value_ids' => $good['spec_value_ids'],
                'discount_price' => $good['discount_price'] ?? 0,
                'is_seckill' => $good['is_seckill'] ?? 0,
                'create_time' => time(),
                'integral_price' => $good['integral_price'] ?? 0,
    ...
    public static function cancel($order_id, $user_id)
    {
        ...
        if ($order['order_type'] == \app\common\model\Order::TEAM_ORDER && $order['pay_status'] == Pay::ISPAID){
