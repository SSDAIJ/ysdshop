...
use app\common\model\{AfterSale, AfterSaleLog, Goods, Order, OrderGoods};
...
class AfterSaleLogic extends LogicBase
{
        ...
        switch ($type) {
            case 'normal':
                ...
                foreach ($lists as $item) {
                    ...
                    $result['time'] = date('Y-m-d H:i', $item['confirm_take_time']);
                    $data[] = $result;
                }
        ...
        foreach ($lists as $k => $item) {
            ...
            $result = [
                ...
                'after_sale' => [
                    ...
                    'desc' => AfterSale::getStatusDesc($item['status']),//todo
                    'able_apply' => 1,//todo
                ],
    ...
    public static function detail($get)
    {
        ...
        $detail = $after_sale
            ->field('id,sn,order_goods_id,refund_reason,refund_type,refund_price,create_time,status')
            ->with(['order_goods'])
