...
use app\common\model\{AfterSale, DistributionOrder, Notice_, Order};
...
class DistributionLogic
{
    ...
    public static function code($code, $my_id)
    {
        try {
            ...
            Hook::listen('notice', [
                ...
                'lower_id' => $my_id,
                'scene' => Notice_::SYSTEM_INVITE_SUCCESS,
            ]);
    ...
    public static function appleDetail($user_id)
    {
        $result = Db::name('distribution_member_apply')
            ->field(['real_name', 'mobile', 'province', 'city', 'district', 'reason', 'status'])
            ->where('user_id', $user_id)
            ->order('id', 'desc')
            ->find();

        //todo  临时解决判断会员问题，现在分销会员后台开通，应该用会标识去判断
        $is_distribution = Db::name('user')
            ->where('id', $user_id)
            ->value('is_distribution');
        if (empty($result) && $is_distribution === 1) {
            $result['province'] = '';
            $result['city'] = '';
            $result['district'] = '';
            $result['real_name'] ='';
    ...
    public static function order($user_id, $get, $page, $size)
    {
        ...
        foreach ($lists as &$item) {
            ...
            $item['image'] = UrlServer::getFileUrl($info['spec_image'] ?? $info['image']);
